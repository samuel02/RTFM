...
{
  NVIC_ISER((uint32_t)(IRQn) >> 5) = (1 << ((uint32_t)(IRQn) & 0x1F)); /* enable interrupt */
}
...
{
  return((uint32_t) ((NVIC_ISPR((uint32_t)(IRQn) >> 5) & (1 << ((uint32_t)(IRQn) & 0x1F)))?1:0)); /* Return 1 if pending else 0 */
}
...
{
  return((uint32_t)((NVIC_IABR((uint32_t)(IRQn) >> 5) & (1 << ((uint32_t)(IRQn) & 0x1F)))?1:0)); /* Return 1 if active else 0 */
}
...
inline void NVIC_SetPriority(IRQn_Type IRQn, uint32_t priority)
{
  ...
  // else {
    NVIC_IP((uint32_t)(IRQn)) = ((priority << (8 - __NVIC_PRIO_BITS)) & 0xff);           /* set Priority for device specific Interrupts  */
  //}
}
...
inline uint32_t NVIC_GetPriority(IRQn_Type IRQn)
{
  ...
  // else {
    return((uint32_t)(NVIC_IP((uint32_t)(IRQn))           >> (8 - __NVIC_PRIO_BITS)));   /* get priority for device specific interrupts  */
  //}
}
