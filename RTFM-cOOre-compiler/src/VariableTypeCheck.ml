...
let typecheck_op env op t1 t2 =
    ...
    match op with
    | OpPlus | OpSub | OpMult | OpDiv | OpMod  -> if in_list rt [Int] then rt else raise (TypeError(type_error_msg rt))
    | OpEq | OpNeq | OpGrt | OpGeq | OpLet | OpLeq  -> if in_list rt [Int;Bool;Byte] then Bool   else raise (TypeError(type_error_msg rt))
