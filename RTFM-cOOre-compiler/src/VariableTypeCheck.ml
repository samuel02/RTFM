...
let rec typecheck_stmt env =  function
    ...
    | If (e, s)         -> if in_list (typecheck_expr env e) [Bool; Int] then typecheck_stmt env s else raise_type_error ("Condition in if-statement must be evaluated to type int or bool.")
    | While (e, s)      -> if in_list (typecheck_expr env e) [Bool; Int] then typecheck_stmt env s else raise_type_error ("Condition in while-statement must be evaluated to type int or bool.")
