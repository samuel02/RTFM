...
let string_of_dl dlp dl =
  string_of_int (pr_of_dl dlp (Time.usec_of_time dl))
...
let ceiling_spec spec dlp =
  ...
  and tops rm il = match il with
    | [] -> rm
    | IIsr (dl, id, s) :: l  ->
      let rm' = stmts (pr_of_dl dlp (Time.usec_of_time dl)) rm s in
      tops rm' l
...
let pl_spec dlp spec rl =
  ...
  and top pl t = match t with
    | []                                -> pl
    | IIsr (dl, id,  _) :: l            -> let pl' = add (pr_of_dl dlp (Time.usec_of_time dl), id ) pl in top pl' l 
    | ITask (_, dl, id, pa,  _, _) :: l -> let pl' = add (pr_of_dl dlp (Time.usec_of_time dl), id ) pl in top pl' l
    | _ :: l                            -> top pl l
